  ...
  setTimeout( function () {
    if ( data.scene[ window.skyes._id ] ) {
      // we have new markers.
        ...
        var _wm = document.createElement( 'a-entity' );
        _wm.setAttribute( 'class', 'link active' );
        let spots = _cm.hotSpot;
        var assets = document.querySelector('a-assets');
        
        for (let i = 0; i < spots.length; i++) {
            var _m = document.createElement( 'a-entity' );
            _m.setAttribute( 'id', 'm-' + window.skyes._id + '-' + spots[i].toScn );
            _m.setAttribute( 'class', 'marker' );
            _m.setAttribute( 'model-opacity', 0 );

            // if ( _cm.type && 'info' == _cm.type ) {

            //   _m.setAttribute( 'obj-model', 'obj: #asterisk-obj; mtl: #asterisk-mtl' );
            //   _m.setAttribute( 'scale', { x: 0.09, y: 0.09, z: 0.09 } );
            //   _m.setAttribute( 'cth-info', 'target: ' + _cm.to + '; pos: ' + _cm.overlay.pos + '; rot: ' + _cm.overlay.rot + '; width: ' + _cm.overlay.width + '; height: ' + _cm.overlay.height + ';' );
            ...
            // }
              ...
              _m.setAttribute( 'obj-model', 'obj: #ar-obj; mtl: #ar-mtl' );
              _m.setAttribute( 'scale', { x: 0.2, y: 0.2, z: 0.2 } );
              addToAssets('img', 'p' + spots[i].toScn, data.scene[ spots[i].toScn ].srcImg, assets);
              _m.setAttribute( 'cth-swap', 'target: ' + spots[i].toScn + ';' );
            


            _m.setAttribute( 'position', spots[i].pos );
            _m.setAttribute( 'rotation', spots[i].rot );
